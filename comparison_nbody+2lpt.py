...
colors = ["black", "MediumOrchid", "#ff7f0e"]
#load particles
all_nbody_positions = []
all_lpt_positions = []
all_emu_positions = []
...
r_max = np.max([np.max(max_radius(all_nbody_voids_incl_small)), np.max(max_radius(all_lpt_voids_incl_small)), np.max(max_radius(all_emu_voids_incl_small))])
print("R_max over all simulations: ", r_max/h, "Mpc")
r_min_global = np.min([np.min(min_radius(all_nbody_voids_incl_small)), np.min(min_radius(all_lpt_voids_incl_small)), np.min(min_radius(all_emu_voids_incl_small))])
...
for i in range(0, n):
    ...
    hist_counts_lpt, bin_edges = np.histogram(lpt_radii_incl_small, bins=bins)
    hist_list_nbody.append(hist_counts_nbody)
    hist_list_lpt.append(hist_counts_lpt)

hist_array_nbody = np.array(hist_list_nbody)
hist_array_lpt = np.array(hist_list_lpt)

hist_mean_nbody = np.mean(hist_array_nbody, axis=0)
hist_mean_lpt = np.mean(hist_array_lpt, axis=0)


hist_error_nbody =  np.sqrt(hist_mean_nbody/n)#np.std(hist_array_nbody, axis=0) 
hist_error_lpt = np.sqrt(hist_mean_lpt/n)# np.std(hist_array_lpt, axis=0) 
...
ax.errorbar(bin_centers, hist_mean_lpt, yerr=hist_error_lpt, fmt='o', color='#ff7f0e', linestyle='-', capsize=5, markersize=3, label='$\mathrm{lpt}$')

resi = np.where(hist_mean_lpt != 0, 
                (hist_mean_lpt - hist_mean_nbody) / np.sqrt(hist_error_lpt**2 + hist_error_nbody**2),
                np.nan)
...
# ---------------------- Plot 1 --------------------------
ax[0].errorbar(bin_centers/h, hist_mean_nbody ,color=colors[0], linestyle='-', capsize=5, markersize=3, label='$N\mathrm{body}$', zorder=2) 
...
ax[0].errorbar(bin_centers/h, hist_mean_lpt, color='#ff7f0e', linestyle='--', capsize=5, markersize=3, label='$\mathrm{2LPT}$', zorder=3)
ax[0].fill_between(bin_centers/h, hist_mean_lpt - hist_error_lpt, hist_mean_lpt + hist_error_lpt, color='#ff7f0e', alpha=0.3)
...
# ----------------------- Plot 2 -------------------------
ax[1].errorbar(bin_centers/h, resi, color='#ff7f0e', linestyle='--', capsize=5, markersize=3, label='$2LPT/Nbody$')
#ax[1].set_ylim(-2, 2)
...
ax[1].axhline(y=0, color='black', ls='-', zorder=5, lw=1, label = '$N\mathrm{body}$')
...
handle_nbody = mlines.Line2D([], [], color='black', linestyle='-', label='$N\mathrm{body}$')
handle_emu   = mlines.Line2D([], [], color='#ff7f0e', linestyle='--', label='$\mathrm{2LPT}$')
...
fig.savefig(f"/u/chahermann/Sparkling/Results/VSF+Ratio_nbody_2lpt_{set}.pdf")
...
# lpt_profiles_list = []
lpt_profiles_list_big = []
lpt_profiles_list_small = []
lpt_profiles_list_mid = []
...
size_settings = {'nbody': {'small': (nbody_profiles_list_small, 'royalblue'),'mid': (nbody_profiles_list_mid, '#FBC02D'), 'big': (nbody_profiles_list_big, '#D32F2F')
    },'lpt': {'small': (lpt_profiles_list_small, 'royalblue'), 'mid':   (lpt_profiles_list_mid, '#FBC02D'), 'big':   (lpt_profiles_list_big,'#D32F2F')   }}


# Schleife über Datasets
for label, (radii_groups, delta_groups) in {
    'nbody': (nbody_void_radii_groups, nbody_delta_profiles_groups),
    'lpt':   (lpt_void_radii_groups,   lpt_delta_profiles_groups)
}.items():
...
fig.savefig(f"/u/chahermann/Sparkling/Results/Stacked_IDP_{set}_2lpt+nbody.pdf")
...
lpt_mean_profile_big = np.mean(lpt_profiles_list_big, axis=0)
lpt_mean_profile_small = np.mean(lpt_profiles_list_small, axis=0)
lpt_mean_profile_mid = np.mean(lpt_profiles_list_mid, axis=0)
...
lpt_std_delta_profile_big = np.std(lpt_profiles_list_big, axis=0) / np.sqrt(len(lpt_profiles_list_big))
lpt_std_delta_profile_small = np.std(lpt_profiles_list_small, axis=0) / np.sqrt(len(lpt_profiles_list_small))
lpt_std_delta_profile_mid = np.std(lpt_profiles_list_mid, axis=0) / np.sqrt(len(lpt_profiles_list_mid))
...
for i in range(0,n):
    lpt_filename=f"{set}_set1_2lpt_run{i:04}"
    nbody_filename=f"{set}_set1_nbody_run{i:04}"
    lpt_delta_groups, lpt_radii_groups = np.load(f"/u/chahermann/2LPT_results/Numpy_files/{lpt_filename}_integrated_delta_profiles_population_spaced_groups.npy", allow_pickle=True)
    ...
    for group in lpt_delta_groups:
        for r, delta in zip(lpt_radii_groups[group], lpt_delta_groups[group]):
            bin_size = get_size_bin(r)  # Funktion zum Ermitteln der Bin-Größe

            if bin_size == 'small':
                file_lpt_small.append(delta)
            
            elif bin_size == 'mid':
                file_lpt_mid.append(delta)

            elif bin_size == 'big':
                file_lpt_big.append(delta)
    ...
    all_lpt_small_means.append(np.mean(file_lpt_small, axis=0))
    all_nbody_small_means.append(np.mean(file_nbody_small, axis=0))
    all_lpt_mid_means.append(np.mean(file_lpt_mid,axis=0))
    all_nbody_mid_means.append(np.mean(file_nbody_mid, axis=0))
    all_lpt_big_means.append(np.mean(file_lpt_big, axis=0))
    all_nbody_big_means.append(np.mean(file_nbody_big,axis=0))
    all_diff_resi_small.append(np.mean(file_lpt_small, axis=0) - np.mean(file_nbody_small, axis=0))
    all_diff_resi_mid.append(np.mean(file_lpt_mid, axis=0) - np.mean(file_nbody_mid, axis=0))
    all_diff_resi_big.append(np.mean(file_lpt_big, axis=0) - np.mean(file_nbody_big, axis=0))
...
lpt_big_means = np.mean(all_lpt_big_means, axis=0)
lpt_mid_means = np.mean(all_lpt_mid_means, axis=0)
lpt_small_means = np.mean(all_lpt_small_means, axis=0)
...
lpt_big_error = np.std(all_lpt_big_means, axis=0)#/np.sqrt(n-1)
nbody_mid_error = np.std(all_nbody_mid_means, axis=0)#/np.sqrt(n-1)
lpt_mid_error = np.std(all_lpt_mid_means, axis=0)#/np.sqrt(n-1)
nbody_small_error = np.std(all_nbody_small_means, axis=0)#/np.sqrt(n-1)
lpt_small_error = np.std(all_lpt_small_means, axis=0)#/np.sqrt(n-1)
...
ax[0].errorbar(scaled_bin_centers, lpt_big_means, linestyle='--', color='#D32F2F', linewidth=1, capsize=3, markersize=6,label="$\mathrm{2LPT:Large \ Voids}$")
ax[0].fill_between(scaled_bin_centers, lpt_big_means - lpt_big_error, lpt_big_means + lpt_big_error, color='#D32F2F', alpha=0.2)

ax[0].errorbar(scaled_bin_centers, lpt_small_means, linestyle='--', color='royalblue', linewidth=1, capsize=3, markersize=6, label="$\mathrm{2LPT:Small \ Voids}$")
ax[0].fill_between(scaled_bin_centers, lpt_small_means - lpt_small_error, lpt_small_means + lpt_small_error, color='royalblue', alpha=0.2)

ax[0].errorbar(scaled_bin_centers, lpt_mid_means, linestyle='--', color='#2E7D32', linewidth=1, capsize=3, markersize=6, label="$\mathrm{2LPT:Medium \ Voids}$")
ax[0].fill_between(scaled_bin_centers, lpt_mid_means - lpt_mid_error, lpt_mid_means + lpt_mid_error, color='#2E7D32', alpha=0.2)
...
handle_nbody = mlines.Line2D([], [], color='black', linestyle='-', label='$N\mathrm{body}$')
handle_emu   = mlines.Line2D([], [], color='black', linestyle='--', label='$\mathrm{2LPT}$')
...
ax[1].set_yticks([-0.5, -0.25, 0, 0.25])

plt.tight_layout()
plt.subplots_adjust(hspace=0.0)
plt.show()
fig.savefig(f"/u/chahermann/Sparkling/Results/IDP+diff_nbody_2lpt_{set}.pdf")
...
lpt_velo_profiles_list_big = []
lpt_velo_profiles_list_small = []
lpt_velo_profiles_list_mid = []
...
lpt_velocity_profiles_groups, lpt_void_radii_groups = np.load(f"/u/chahermann/2LPT_results/Numpy_files/all_velocity_profiles_equipopulated_groups_{set}.npy", allow_pickle=True)
...
size_settings = {'nbody': {'small': (nbody_velo_profiles_list_small, 'royalblue'),'mid': (nbody_velo_profiles_list_mid, '#FBC02D'), 'big': (nbody_velo_profiles_list_big, '#D32F2F')
    }, 'lpt': {'small': (lpt_velo_profiles_list_small, 'royalblue'), 'mid':   (lpt_velo_profiles_list_mid, '#FBC02D'), 'big':   (lpt_velo_profiles_list_big,'#D32F2F') }}

# Schleife über Datasets
for label, (radii_groups, velo_groups) in {'nbody': (nbody_void_radii_groups, nbody_velocity_profiles_groups),'lpt':   (lpt_void_radii_groups,   lpt_velocity_profiles_groups)
}.items():
...
legend_handles = [ax.plot([], [], color=colors[0], linestyle='-', label='$Nbody$', alpha=1)[0], 
                  #ax.plot([], [], color=colors[1], linestyle='-', label='$2LPT$', alpha=1)[0],
                  ax.plot([], [], color=colors[2], linestyle='-', label='$2LPT$', alpha=1)[0]]
ax.legend(handles=legend_handles, fontsize=18, framealpha=1)
ax.get_yaxis().set_tick_params(which='both', direction='in', labelsize=15)
ax.get_xaxis().set_tick_params(which='both', direction='in', labelsize=15)
ax.axhline(y=0, color='k', ls='-', zorder=5, lw=0.5)
...
legend_handles = [ax.plot([], [], color=colors[0], linestyle='-', label='$\mathrm{Nbody}$', alpha=1)[0], 
                 # ax.plot([], [], color=colors[1], linestyle='-', label='$2LPT$', alpha=1)[0],
                  ax.plot([], [], color=colors[2], linestyle='-', label='$\mathrm{2LPT}$', alpha=1)[0]]
ax.legend(handles=legend_handles, fontsize=18, framealpha=1)
...
lpt_velo_mean_profile_big = np.nanmean(lpt_velo_profiles_list_big, axis=0)
lpt_velo_mean_profile_small = np.nanmean(lpt_velo_profiles_list_small, axis=0)
lpt_velo_mean_profile_mid = np.nanmean(lpt_velo_profiles_list_mid, axis=0)
...
lpt_std_velo_profile_big = np.nanstd(lpt_velo_profiles_list_big, axis=0) / np.sqrt(len(lpt_velo_profiles_list_big))
lpt_std_velo_profile_small = np.nanstd(lpt_velo_profiles_list_small, axis=0) / np.sqrt(len(lpt_velo_profiles_list_small))
lpt_std_velo_profile_mid = np.nanstd(lpt_velo_profiles_list_mid, axis=0) / np.sqrt(len(lpt_velo_profiles_list_mid))
...
print("Shape emu small:", lpt_velo_mean_profile_small.shape)
print("Shape scaled bins:", scaled_bin_centers.shape)

ax.errorbar(scaled_bin_centers, lpt_velo_mean_profile_big, yerr=lpt_std_velo_profile_big, fmt = 'o', linestyle='-', color=colors[2], linewidth=2, capsize= 3, markersize =3, label="Emu: Big Voids")
ax.errorbar(scaled_bin_centers, lpt_velo_mean_profile_small, yerr=lpt_std_velo_profile_small, fmt = 'o', linestyle='-', color='lightblue', linewidth=2, capsize= 3, markersize =3, label="Emu: Small Voids")
ax.errorbar(scaled_bin_centers, lpt_velo_mean_profile_mid, yerr=lpt_std_velo_profile_mid, fmt = 'o', linestyle='-', color='lightblue', linewidth=2, capsize= 3, markersize =3, label="Emu: Small Voids")
...
all_lpt_small_velo_means = []
all_nbody_small_velo_means = []
all_lpt_mid_velo_means = []
all_nbody_mid_velo_means = []
all_lpt_big_velo_means = []
...
for i in range(0,n):
    lpt_filename=f"{set}_set1_2lpt_run{i:04}"
    nbody_filename=f"{set}_set1_nbody_run{i:04}"
    lpt_velo_groups, lpt_radii_groups = np.load(f"/u/chahermann/2LPT_results/Numpy_files/{lpt_filename}_velocity_profiles_equipopulated_groups.npy", allow_pickle=True)
    ...
    #body_velo_groups, nbody_radii_groups = np.load(f"/u/chahermann/Nbody_results/Numpy_files/rev_{nbody_filename}_velocity_profiles_equipopulated_groups.npy", allow_pickle=True)
    file_lpt_small_velo = []
    ...
    for group in lpt_velo_groups:
        for r, velo in zip(lpt_radii_groups[group], lpt_velo_groups[group]):
            bin_size = get_size_bin(r)  # Funktion zum Ermitteln der Bin-Größe

            if bin_size == 'small':
                file_lpt_small_velo.append(velo)
            
            elif bin_size == 'mid':
                file_lpt_mid_velo.append(velo)

            elif bin_size == 'big':
                file_lpt_big_velo.append(velo)
    ...
    all_lpt_small_velo_means.append(np.nanmean(file_lpt_small_velo, axis=0))
    all_nbody_small_velo_means.append(np.nanmean(file_nbody_small_velo, axis=0))
    all_lpt_mid_velo_means.append(np.nanmean(file_lpt_mid_velo,axis=0))
    all_nbody_mid_velo_means.append(np.nanmean(file_nbody_mid_velo, axis=0))
    all_lpt_big_velo_means.append(np.nanmean(file_lpt_big_velo, axis=0))
    all_nbody_big_velo_means.append(np.nanmean(file_nbody_big_velo,axis=0))
    all_diff_resi_velo_small.append(np.nanmean(file_lpt_small_velo, axis=0) - np.nanmean(file_nbody_small_velo, axis=0))
    all_diff_resi_velo_mid.append(np.nanmean(file_lpt_mid_velo, axis=0) - np.nanmean(file_nbody_mid_velo, axis=0))
    all_diff_resi_velo_big.append(np.nanmean(file_lpt_big_velo, axis=0) - np.nanmean(file_nbody_big_velo, axis=0))
...
lpt_big_velo_means = np.nanmean(all_lpt_big_velo_means, axis=0)
lpt_mid_velo_means = np.nanmean(all_lpt_mid_velo_means, axis=0)
lpt_small_velo_means = np.nanmean(all_lpt_small_velo_means, axis=0)

nbody_big_velo_error = np.nanstd(all_nbody_big_velo_means, axis=0)#/np.sqrt(n-1)
lpt_big_velo_error = np.nanstd(all_lpt_big_velo_means, axis=0)#/np.sqrt(n-1)
nbody_mid_velo_error = np.nanstd(all_nbody_mid_velo_means, axis=0)#/np.sqrt(n-1)
lpt_mid_velo_error = np.nanstd(all_lpt_mid_velo_means, axis=0)#/np.sqrt(n-1)
nbody_small_velo_error = np.nanstd(all_nbody_small_velo_means, axis=0)#/np.sqrt(n-1)
lpt_small_velo_error = np.nanstd(all_lpt_small_velo_means, axis=0)#/np.sqrt(n-1)
...
ax[0].errorbar(scaled_bin_centers, lpt_big_velo_means, linestyle='--', color='#D32F2F', linewidth=1, capsize=3, markersize=6,label="$\mathrm{2LPT:Large \ Voids}$")
ax[0].fill_between(scaled_bin_centers, lpt_big_velo_means - lpt_big_velo_error, lpt_big_velo_means + lpt_big_velo_error, color='#D32F2F', alpha=0.2)

ax[0].errorbar(scaled_bin_centers, lpt_small_velo_means, linestyle='--', color='royalblue', linewidth=1, capsize=3, markersize=6, label="$\mathrm{2LPT:Small \ Voids}$")
ax[0].fill_between(scaled_bin_centers, lpt_small_velo_means - lpt_small_velo_error, lpt_small_velo_means + lpt_small_velo_error, color='royalblue', alpha=0.2)

ax[0].errorbar(scaled_bin_centers, lpt_mid_velo_means, linestyle='--', color='#2E7D32', linewidth=1, capsize=3, markersize=6, label="$\mathrm{2LPT:Medium \ Voids}$")
ax[0].fill_between(scaled_bin_centers, lpt_mid_velo_means - lpt_mid_velo_error, lpt_mid_velo_means + lpt_mid_velo_error, color='#2E7D32', alpha=0.2)
...
handle_nbody = mlines.Line2D([], [], color='black', linestyle='-', label='$N\mathrm{body}$')
handle_lpt   = mlines.Line2D([], [], color='black', linestyle='--', label='$\mathrm{2LPT}$')

combined_handles = [handle_small, handle_mid, handle_big, handle_nbody, handle_lpt]
...
ax[0].set_yticks([ -20, 0, 20, 40, 60, 80])

plt.tight_layout()
plt.subplots_adjust(hspace=0.0)
fig.savefig(f"/u/chahermann/Sparkling/Results/VP+diff_nbody_2lpt_{set}.pdf")
