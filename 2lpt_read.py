...
#create ascii files for all 2lpt_0000*.bin files
for i in range(0,n):
    filename = f"{set}_set1_2lpt_run{i:04}"
    j=i+1
    particles_data = GadgetReader(f'/ptmp/ccorrea/Aletheia-ML/{set}/set1/2lpt/runs/output_run06{j:02}/snapshot_ics_000')   
    positions = particles_data['pos'].T  # Transpose to get the right shape
    velocities = particles_data['vel'].T
    print(f"Shape of lpt_positions: {positions.shape}")

    data_to_save = np.hstack((positions, velocities))
    #print(data_to_save.shape)
    np.savetxt(f"/u/chahermann/Sparkling/input/2lpt_input/{filename}.dat", data_to_save, fmt="%.6f", delimiter = "\t", comments="")