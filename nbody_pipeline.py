...
example = np.loadtxt(f"/u/chahermann/Sparkling/input/nbody_input/{set}_set1_nbody_run0000.dat")
...
for i in range(0, n):
    filename = f"{set}_set1_nbody_run{i:04}"
    void_key = f"void_data_incl_small_{filename}"

    #void_data_incl_small[void_key] = voidfinder_reader(f'/u/chahermann/Revolver/output/nbody_output/{filename}/zobov-Revolver_Voids_cat.txt')
    void_data_incl_small[void_key] = voidfinder_reader(f'/u/chahermann/Sparkling/output/nbody_output/void_{filename}.dat')
...
#fig.suptitle("$ \mathrm{Void \ And \ DM \ Distribution \ Comparison \ For \ The \ Nbody \ Data} $", fontsize=20, y=0.95)

z_min = 0
z_max = 5
filename = f"{set}_set1_nbody_run0000"

void_key = f"void_data_incl_small_{filename}"
radii_incl_small, coords_incl_small = void_data_incl_small[void_key]

data_particles = np.loadtxt(f'/u/chahermann/Sparkling/input/nbody_input/{filename}.dat')
...
# ax.set_xlim(xlim)
# ax.set_ylim(ylim)
# fig.savefig(f"/u/chahermann/Nbody_results/Plots/sparkling_spat_nbody_{set}.pdf")
fig.savefig(f"/u/chahermann/Nbody_results/Plots/sparkling_spat_nbody_{set}.png", dpi=100, bbox_inches='tight', pad_inches=0)
...
ax.errorbar(bin_centers, hist_nbody/(n-1), yerr=errors_nbody, fmt='o', color='green', linestyle='-', capsize=5, markersize=3, label='$Nbody$')
...
ticks = np.logspace(np.log10(min(all_nbody_radii_incl_small)), np.log10(max(all_nbody_radii_incl_small)), 4)
...
fig.savefig(f"/u/chahermann/Nbody_results/Plots/sparkling_VSF_nbody_means_{set}.pdf")
...
for i in range(0, n):
    filename = f"{set}_set1_nbody_run{i:04}"
    particles_data = np.loadtxt(f"/u/chahermann/Sparkling/input/nbody_input/{filename}.dat")
...
for i in range(0, n):
    ...
    #np.save(f"/u/chahermann/Nbody_results/Numpy_files/rev_{filename}_integrated_delta_profiles_population_spaced_groups.npy", (delta_profiles_groups, radii_groups))
    np.save(f"/u/chahermann/Nbody_results/Numpy_files/{filename}_integrated_delta_profiles_population_spaced_groups_{set}.npy", (delta_profiles_groups, radii_groups))
...
np.save(f"/u/chahermann/Nbody_results/Numpy_files/all_integrated_delta_profiles_population_spaced_groups_{set}.npy", (all_delta_profiles_groups, all_radii_groups))
#np.save("/u/chahermann/Nbody_results/Numpy_files/rev_all_integrated_delta_profiles_population_spaced_groups.npy", (all_delta_profiles_groups, all_radii_groups))


# In[ ]:


# Replace the existing plotting section for IDP with:
...
filename = f"{set}_set1_nbody_run0001"
delta_profiles_groups, radii_groups = np.load(
    f"/u/chahermann/Nbody_results/Numpy_files/{filename}_integrated_delta_profiles_population_spaced_groups_{set}.npy", 
    allow_pickle=True
)

# Directly plot each profile
for group in delta_profiles_groups:
    for delta_profile in delta_profiles_groups[group]:
        ax.plot(scaled_bin_centers, delta_profile, 
                marker='None', linestyle='-', 
                color=colors[group], 
                alpha=0.01)  # Use very low alpha for many overlapping lines
            
    ax.set_ylim(-1.1, 0.3)
    ax.tick_params(axis='both', which='both', direction='in', labelsize=20)
    ax.set_xlabel("$R/R_v$", fontsize=20)
    ax.set_ylabel("$\\Delta(R)$", fontsize=20)
    ax.set_title("$Void \ Density \ Profile \ For \ Nbody$", fontsize=23)
    ax.grid(True, linestyle='--', alpha=0.6)
    ax.axhline(y=0, color='k', ls='-', zorder=5, lw=0.5)
    ax.axvline(x=1, color='k', ls='--', zorder=5)
    ax.axhline(y=-0.7, color='k', ls='--', zorder=5)

dummy_handles = []
for group in sorted(delta_profiles_groups.keys()):
    line, = ax.plot([], [], color=colors[group], 
                    label=f'$[{limits[group-1]}, \ {limits[group]}]$'+' $h^{-1} \ \\mathrm{Mpc}$')
    dummy_handles.append(line)

ax.legend(handles=dummy_handles, fontsize=20, framealpha=1, 
              loc='upper right', title="$Group \ Limits$", title_fontsize=18)

plt.tight_layout()
fig.savefig(f"/u/chahermann/Nbody_results/Plots/sparkling_IDP_nbody_{set}.pdf")
...
all_delta_profiles_groups, all_radii_groups = np.load(f"/u/chahermann/Nbody_results/Numpy_files/all_integrated_delta_profiles_population_spaced_groups_{set}.npy", allow_pickle=True)
#all_delta_profiles_groups, all_radii_groups = np.load("/u/chahermann/Nbody_results/Numpy_files/rev_all_integrated_delta_profiles_population_spaced_groups.npy", allow_pickle=True)
...
fig.savefig(f"/u/chahermann/Nbody_results/Plots/sparkling_IDP_nbody_mean_{set}.pdf")
...
for i in range(0, n):
    ...
    particles_data = np.loadtxt(f"/u/chahermann/Sparkling/input/nbody_input/{filename}.dat")
    positions = particles_data[:, :3]
    positions = apply_pbc(positions, box)  # <- falls nötig, nicht schädlich
    tree = cKDTree(positions, boxsize=box + 1e-13)
    ...
    for R_v_i, center_i in zip(voids[:, 0], voids[:, 1:4]):
        ...
        vecs = minimum_image(vecs, box)  # ← PBC hier!
    ...
    # Speichern pro File
    np.save(
        f"/u/chahermann/Nbody_results/Numpy_files/{filename}_differential_delta_profiles_equipopulated_groups_{set}.npy",
        # f"/u/chahermann/Nbody_results/Numpy_files/rev_{filename}_differential_delta_profiles_equipopulated_groups.npy",
...
np.save(f"/u/chahermann/Nbody_results/Numpy_files/all_differential_delta_profiles_equipopulated_groups_{set}.npy", (all_delta_profiles_groups, all_radii_groups))
# np.save("/u/chahermann/Nbody_results/Numpy_files/rev_all_differential_delta_profiles_equipopulated_groups.npy", (all_delta_profiles_groups, all_radii_groups))
...
for i, ax in enumerate(axes[:4], start=1):
    filename = f"{set}_set1_nbody_run{i:04}"
    delta_profiles_groups, radii_groups = np.load(f"/u/chahermann/Nbody_results/Numpy_files/{filename}_differential_delta_profiles_equipopulated_groups_{set}.npy", allow_pickle=True)
    #delta_profiles_groups, radii_groups = np.load(f"/u/chahermann/Nbody_results/Numpy_files/rev_{filename}_differential_delta_profiles_equipopulated_groups.npy", allow_pickle=True)
    ...
    ax.set_xlabel("$R/R_v$", fontsize=20)
    ax.set_ylabel("$\Delta(R)$", fontsize=20)
    ax.set_title(rf"$Void \ Density \ Profile \ For \ Nbody{i}$", fontsize=23)
...
all_delta_profiles_groups, all_radii_groups = np.load(f"/u/chahermann/Nbody_results/Numpy_files/all_differential_delta_profiles_equipopulated_groups_{set}.npy", allow_pickle=True)
# all_delta_profiles_groups, all_radii_groups = np.load("/u/chahermann/Nbody_results/Numpy_files/rev_all_differential_delta_profiles_equipopulated_groups.npy", allow_pickle=True)
...
plt.title('$ \mathrm{Differential \ Void \ Density \ Profiles \ Means \ Nbody} $', fontsize=22)
plt.xlabel('$R/R_v$', fontsize=22)
plt.ylabel('$\delta(R)$', fontsize=22)
...
for i in range(0, n):
    ...
    #save each file
    np.save(
        f"/u/chahermann/Nbody_results/Numpy_files/{filename}_velocity_profiles_equipopulated_groups_{set}.npy",
        #f"/u/chahermann/Nbody_results/Numpy_files/rev_{filename}_velocity_profiles_equipopulated_groups.npy",
    ...
    for group in velocity_profiles_groups:
        ...
        all_radii_groups[group].extend(radii_groups[group])
#save for all files
np.save(f"/u/chahermann/Nbody_results/Numpy_files/all_velocity_profiles_equipopulated_groups_{set}.npy", (all_velocity_profiles_groups, all_radii_groups))
#np.save("/u/chahermann/Nbody_results/Numpy_files/rev_all_velocity_profiles_equipopulated_groups.npy", (all_velocity_profiles_groups, all_radii_groups))
...
for i in range(0,4):
    filename = f"{set}_set1_nbody_run{i:04}"
    velocity_profiles_groups, radii_groups = np.load(f"/u/chahermann/Nbody_results/Numpy_files/{filename}_velocity_profiles_equipopulated_groups_{set}.npy", allow_pickle=True)
    #velocity_profiles_groups, radii_groups = np.load(f"/u/chahermann/Nbody_results/Numpy_files/rev_{filename}_velocity_profiles_equipopulated_groups.npy", allow_pickle=True)
    ax = axs[i-1]
    for group in velocity_profiles_groups:
    ...
    ax.set_title(f"$Velocity \ Profile \ For \ Nbody{i}$", fontsize=18)
...
fig.suptitle("$ \mathrm{Void \ Velocity \ Profiles \ Nbody} $", fontsize=30)
plt.tight_layout()
plt.subplots_adjust(top=0.92)
fig.savefig(f"/u/chahermann/Nbody_results/Plots/sparkling_VP_nbody_{set}.pdf")
...
#all_velocity_profiles_groups, all_radii_groups = np.load("/u/chahermann/Nbody_results/Numpy_files/rev_all_velocity_profiles_equipopulated_groups.npy", allow_pickle=True)
all_velocity_profiles_groups, all_radii_groups = np.load(f"/u/chahermann/Nbody_results/Numpy_files/all_velocity_profiles_equipopulated_groups_{set}.npy", allow_pickle=True)
...
plt.title('$\mathrm{Velocity \ Profiles \ Means \ Nbody}$', fontsize=20)
...
plt.tight_layout()
fig.savefig(f"/u/chahermann/Nbody_results/Plots/sparkling_VP_nbody_means_{set}.pdf")
